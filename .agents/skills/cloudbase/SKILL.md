---
name: cloudbase
description: "Use this skill for CloudBase (TCB, Tencent 云开发, 微信云开发) development, deployment, debugging, migration, and troubleshooting. Covers Web apps (React, Vue, Vite, Next, Nuxt), WeChat Mini Programs, uni-app, native/mobile via HTTP API, UI/page design, auth/login/provider setup, NoSQL/MySQL CRUD and security rules, cloud functions, CloudRun, cloud storage, static hosting, built-in AI models (generateText, streamText, createModel, generateImage, TokenHub, Hunyuan, DeepSeek, GLM, Kimi), third-party LLMs, AI agents, AG-UI, CloudBase CLI, ops inspection, logs, diagnostics, and spec workflow."
